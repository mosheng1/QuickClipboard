...
pub use services::{
    get_data_directory, get_settings, hotkey, update_settings, AppSettings, AppSounds, SoundPlayer,
};
...
pub use windows::main_window::{
    check_snap, get_main_window, get_window_state, hide_main_window, hide_snapped_window,
    init_edge_monitor, is_dragging, is_main_window_visible, is_window_snapped, restore_from_snap,
    set_window_state, show_main_window, show_snapped_window, snap_to_edge, start_drag, stop_drag,
    toggle_main_window_visibility, SnapEdge, WindowState,
};
    ...
    #[cfg(windows)]
    {
        ...
        if let Ok(settings) = services::settings::load_settings_from_file() {
            if settings.run_as_admin {
                ...
                services::system::startup::repair_startup_configuration(settings.auto_start, false)
            {
        ...
        {
            eprintln!(
                "[dev] 开发模式：跳过管理员提权检查（run_as_admin 设置在 release 模式下生效）"
            );
        }
    }
    ...
    let app = builder
        ...
        .setup(|app| {
            ...
            if let Err(e1) = services::database::init_database(db_path_str) {
                ...
                services::database::init_database(db_path_str).map_err(|e2| {
                    format!("数据库初始化失败(已尝试清理 wal/shm): {} -> {}", e1, e2)
                })?;
            }
            ...
            let settings_exists =
                services::settings::storage::SettingsStorage::exists().unwrap_or(true);
            if !settings_exists {
