...
截图功能通过 feature `screenshot-suite` 统一控制（`lib.rs` 中使用 `#[cfg(feature = "screenshot-suite")]` 条件编译）。
完整构建启用此 feature；社区构建通过 `--no-default-features` 禁用此 feature。

`capabilities/screenshot.json` 中使用 `screenshot-suite:default` 权限前缀，由截图插件自身注册。

> ⚠️ 社区构建通过 `scripts/community-build.js` 临时修改 `Cargo.toml` 移除私有依赖行并传递 `--no-default-features`，构建后自动恢复。`lib.rs` 中当前**没有** `compile_error!` 来强制私有与社区版互斥，仅靠构建流程保证。
...
- ❌ 在社区版中启用私有 feature：`screenshot-suite`、`gpu-image-viewer`
